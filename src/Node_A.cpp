...
public:
...

    {
        ...
        move_group.setEndEffectorLink("gripper_grasping_frame_Z");
		initialize_routines(); 
    ...
    void AprilTagDetectionCallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr& msg) 
    {
        ...
        for(const auto& detection : msg->detections)
        {
            ...
            if(id_tag==10 && line_origin.header.frame_id.empty())
            {
                ...
                line_origin.pose.position.z = 0.78; // fix the table frame at the same heiight of the table
	...
	void initialize_routines()
    {
        ...
        goal.target_pose.pose.position.y = -2.23049; // y-coordinate
...
/*
    ...
    */
    ...
    void approach(geometry_msgs::Pose& pose) {
        ...
        // Adjust the target pose to be 30 cm above the marker
    ...
    void Placing(int i)
    {
        ...
        position.push_back({0.0, 0.0}); // Look front 
        moveHead(position); 
        ros::Duration(1.0).sleep();
        position.clear();
...
private:
    ...
    std::vector<double> coordinates = {0.0,0.1,0.2};
